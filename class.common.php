...
//----------------------------------------------------------
 ...
 //----------------------------------------------------------
    ...
    public function getDiscount_name($discount_name){
        ...
        $query ="SELECT * FROM discount WHERE (discount_name ='{$discount_name}' OR discount_code ='{$discount_name}') AND
        active = 1 LIMIT 1";
        $result = mysqli_query($this->con,$query);
        $list = "";
        if($result){
    ...
    function US_format_phone($phone){
        $numbers = explode("\n",$phone);
       return preg_replace('~.*(\d{3})[^\d]{0,7}(\d{3})[^\d]{0,7}(\d{4}).*~', '($1) $2-$3', $numbers). "\n";
  ...
  //--------------------------
    public function get_total_payment_sales($pay_order){
        $query ="SELECT SUM(pay_amount) as total FROM pay_salesperson
         WHERE pay_order='$pay_order'";
        $result = mysqli_query($this->con,$query);
    ...
    //------------------------------------------------------
    public function sales_paid_list($pay_salesperson)
    {
        $sqlText = "Select * From paid_salesperson_short
        WHERE pay_salesperson='{$pay_salesperson}' order by pay_order DESC ";
