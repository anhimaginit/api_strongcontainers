...
class Task extends Common{
                               ...
                               $file_pickup_name=null,$file_delivery_name=null)
    {
        ...
        if($deliverydate !='' && $assign_driver_id !=''){
            $available = $obj_calendar->check_driver_in_calendar($assign_driver_id,$deliverydate,'',$assign_order);
            if($available["driver"] !=''){
        ...
        if($assign_order !=''){
            ...
            if($assign_driver_id !=''){
               ...
               $driver_name = $this->get_driver_name($assign_driver_id);
                $fields .= ",driver_name";
                $values .=",'{$driver_name}'";
            }
        }else{
                               ...
                               $file_pickup_name=null, $file_delivery_name=null)
    {
        ...
        $available =array();
        if($deliverydate !='' && $assign_driver_id !=''){
            $available = $obj_calendar->check_driver_in_calendar($assign_driver_id,$deliverydate,$id,$assign_order);
        ...
        if($assign_order !=''){
            ...
            if($assign_driver_id !=''){
                $driver_name = $this->get_driver_name($assign_driver_id);
                $updateComm .=",driver_name = '{$driver_name}'";
   ...
   //------------------------------------------------------------
    ...
    public function get_driver_name($driver_id){
        $query ="SELECT trim(concat(ifnull(trim(`first_name`),''),if(((`middle_name` is not null) and (`middle_name` <> '')),concat(' ',trim(`middle_name`)),''),if(((`last_name` is not null) and (`last_name` <> '')),concat(' ',trim(`last_name`)),''))) AS `driver_name`
        FROM `contact`
        WHERE ID ='{$driver_id}'";
        $result = mysqli_query($this->con,$query);
