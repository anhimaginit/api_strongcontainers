...
class Calendar extends Common{
    public function  check_driver_in_calendar($assign_id,$delivery_date,$task_id=null,$order=null){
        if($delivery_date ==''){
        ...
        if($task_id !=''){
            $query ="SELECT assign_order FROM calendar_short
        WHERE task_id = '{$task_id}'";
            ...
            if($rsl){
                while ($row = mysqli_fetch_assoc($rsl)) {
                    $assign_order = $row['assign_order'];
                    $sku_list = $this->return_id("SELECT products_ordered FROM quote WHERE order_id ='{$assign_order}'","products_ordered");
                    $row["qty"]=0;
                    if($sku_list !=''){
                        $sku_list = json_decode($sku_list,true);
                        $row["qty"]=$sku_list[0]['quantity'];
                    }
                    $list = $row;
                }
            }
            if(count($list) >0){
                if($list['assign_order'] !=$order){
                    $sku_list = $this->return_id("SELECT products_ordered FROM quote WHERE order_id ='{$order}'","products_ordered");
                    $qty=0;
                    if($sku_list !=''){
                        $sku_list = json_decode($sku_list,true);
                        $qty=$sku_list[0]['quantity'];
                    }

                    if($qty >$list['qty']){
                        $query ="SELECT delivery_date,calendar_id,task_id,task_in_day,assign_order FROM calendar_short
