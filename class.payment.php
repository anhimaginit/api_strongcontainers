...
//-------------------------------------------------
    ...
    //------------------------------------------------
    public function new_pay_salesperson($pay_salesperson,$pay_amount,$pay_order,$pay_date,
                                   $submit_by,$pay_type,$pay_note,$transaction)
    {
        ...
        $fields = ""; $values='';
        if($pay_salesperson !=''){
            $fields .="pay_salesperson";
            $values .="'{$pay_salesperson}'";
        }
        ...
        $insertCommand = "INSERT INTO pay_salesperson ({$fields}) VALUES({$values})";
        //die($insertCommand);
        mysqli_query($this->con,$insertCommand);
        $idreturn = mysqli_insert_id($this->con);
        if(is_numeric($idreturn) && $idreturn !=''){
    ...
    //------------------------------------------------------
    public function get_paid_sales_order($order_id)
    {
        $query = "SELECT q.salesperson_amount, q.s_ID,q.s_name,
            x.pay_order,x.salesperson_paid
         FROM quote_short AS q
                      INNER JOIN (SELECT
                      SUM(p.pay_amount) as salesperson_paid,
                      p.pay_order
