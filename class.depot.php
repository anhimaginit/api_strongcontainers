...
class Depot extends Common{
    ...
    //-------------------------------
    public function depots_short($zip,$container_type_id,$container_feet_type,$find_depot){
        if($find_depot =='find_depot'){
            if($zip !='') {
                $query ="SELECT depot_id,depot_address,depot_city,depot_state,depot_zip
                  FROM `depots_short` WHERE depot_zip = '{$zip}'
                 GROUP BY depot_id";

                $query_short ="SELECT * FROM `depots_short` WHERE depot_zip = '{$zip}' ";
                if($container_feet_type !=''){
                    $query .=" AND container_feet_type ='{$container_feet_type}'";
                    $query_short .=" AND container_feet_type ='{$container_feet_type}'";
                }

                $query_short .=" ORDER BY depot_name";
            } else{
                //$query ="SELECT depot_id,depot_address FROM `depots_short` GROUP BY depot_id";
                $query ="SELECT DISTINCT depot_id,depot_address,depot_city,depot_state,depot_zip
                FROM `depots_short`";
                $query_short ="SELECT * FROM `depots_short`";
                if($container_feet_type !=''){
                    $query .=" WHERE container_feet_type ='{$container_feet_type}'";
                    $query_short .=" WHERE container_feet_type ='{$container_feet_type}'";
                }
                $query_short .=" ORDER BY depot_name";
            }
        }else{
            if($zip !='') {
                $query ="SELECT depot_id,depot_address,depot_city,depot_state,depot_zip
             FROM `depots_short` WHERE depot_zip = '{$zip}' AND
                product_qty > 0
                 GROUP BY depot_id";

                $query_short ="SELECT * FROM `depots_short` WHERE depot_zip = '{$zip}' AND
            product_qty > 0 ";
                if($container_feet_type !=''){
                    $query .=" AND container_feet_type ='{$container_feet_type}'";
                    $query_short .=" AND container_feet_type ='{$container_feet_type}'";
                ...
                //$query ="SELECT depot_id,depot_address FROM `depots_short` GROUP BY depot_id";
                $query ="SELECT DISTINCT depot_id,depot_address,depot_city,depot_state,depot_zip
            FROM `depots_short` WHERE product_qty > 0 ";
                $query_short ="SELECT * FROM `depots_short` WHERE product_qty > 0 ";
                if($container_feet_type !=''){
                    $query .=" AND container_feet_type ='{$container_feet_type}'";
                    $query_short .=" AND container_feet_type ='{$container_feet_type}'";
                }
    ...
    //-------------------------------
    public function add_quote_temp($cus_name,$email_phone, $cus_address, $cus_city,
        ...
        $cus_zipcode = trim($cus_zipcode);
        $cus_phone = trim($cus_phone);
        //create quote_temp
        $code = '';
        $values ="";
        if($email_phone !=''){
            $code =base64_encode($email_phone.$max_id);
        }elseif($cus_phone !=''){
            $code =base64_encode($cus_phone.$max_id);
        }elseif(empty($email_phone) && empty($cus_phone)) return array("error"=>"Email and Phone is not empty","quote_temp_id"=>'');

        if(count($data) <1) return array("error"=>"Data require","quote_temp_id"=>'');


        $values ="'{$cus_phone}','{$code}','{$cus_address}',
        '{$cus_city}','{$cus_zipcode}','{$cus_state}',
        '{$cus_phone}','{$cus_name}','New'";

        $fields ="email_phone,code,shipping_address,
         shipping_city,shipping_zip,shipping_state,
         shipping_phone,shipping_customer_name,status";
    ...
    //-------------------------------
    public function add_quote_temp_bk($email,$phone, $data){
        $query = "SELECT MAX(quote_temp_id) as max_id  FROM `quote_temp` LIMIT 1";
        $result = mysqli_query($this->con,$query);
        $max_id =1;
        if($result){
    ...
    public function saveCustomerInfo($first_name,$primary_street_address1,
                                     ...
                                     $tran_id,$payment_type,$amount,$discount_code=null){
        ...
        if($discount_code !=''){
           $discount_info = $this->check_discount_code($discount_code);
            if($discount_info['discount'] !=''){
                if($discount_info['discount_type'] =='$'){
                    $total = $total- $discount_info['discount'];
                }else{
                    $total = $total - ($total * $discount_info['discount'])/100;
                }
            }
        }
        //
    ...
    //-------------------------------
    public function search_rate_container($limit,$cursor,$text_search,$depot_id){
        $query ="SELECT *
        FROM `rate_container_short` WHERE container_active =1";
    ...
    public function get_depot_id($depot_id){
        ...
        if($result){
            while ($row = mysqli_fetch_assoc($result)) {
                $row['product_list'] = $this->get_product_depot($depot_id);
                $list =$row;
            }
        }
        return $list;
    ...
    public function auto_update_sku()
    {
        ...
        $sqlText = "Select * From `rate_container_short`";
        $result = mysqli_query($this->con,$sqlText);
        $list = array();
        if($result){
            while ($row = mysqli_fetch_assoc($result)) {
        ...
        if(count($list) > 0){
            foreach($list as $item){
                $item['container_type_name'] = preg_replace("/`+/", '', $item['container_type_name']);
                $item['container_type_name'] = preg_replace("/'+/", '', $item['container_type_name']);
    ...
    //-------------------------------
    public function get_product_depot($depot_id){
        $query ="SELECT *
        FROM `depots_short` WHERE depot_id  ='{$depot_id}'";
